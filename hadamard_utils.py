...
def apply_exact_had_to_linear(module, had_dim=-1, output=False):
    ...
    dev = W_.device
    init_shape = W_.shape
    W_ = W_.double().to(device='cpu')
    
    if had_dim == -1:
        if output:
            had_K, K = get_hadK(out_features)
            W_ = matmul_hadU(W_.t()).t()
        if not output:
        ...
        if output:
            ...
            # W_reshaped = W_.reshape(-1, transposed_shape[-1]//had_dim, had_dim).to(device='cpu', dtype=torch.float64)
            # # W_ = F.linear(W_reshaped, torch.tensor(scipy.linalg.hadamard(had_dim), dtype=torch.float64)) / math.sqrt(had_dim) # very bad mse
            # W_ = F.linear(W_reshaped, hadK) / math.sqrt(had_dim)
